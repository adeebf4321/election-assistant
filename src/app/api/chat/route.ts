...
/**
 * Attempts to call the Gemini API with automatic retry.
 * Falls back to offline knowledge base if all retries fail.
 */
async function callGeminiWithRetry(
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      // All retries exhausted or non-retryable error — break out
      break;
    }
  }
...
export async function POST(req: Request) {
  try {
    ...
    const reply = await callGeminiWithRetry(message, formattedHistory);
    return NextResponse.json({ reply });
  } catch (error) {
    console.error("Chat API Error:", error);
    // Even on unexpected errors, still return a useful response
    return NextResponse.json({
      reply: "I encountered an unexpected issue. Here is some general guidance:\n\n" +
        getOfflineResponse("")
    });
  }
}
